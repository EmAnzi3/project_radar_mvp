...
function matchesMainFilters(p){
  ...
  const region=$('region')?.value||'',site=$('siteType')?.value||'',stage=$('stage')?.value||'',type=$('type')?.value||'',developer=$('developer')?.value||'',contractor=$('contractor')?.value||'';
  ...
  if(region&&p.region!==region||site&&siteType!==site||stage&&p.stage!==stage||type&&p.type!==type||developer&&p.developer!==developer)return false;
...
function renderSelection(){const box=$('provinceMapSelection');if(!box)return;box.innerHTML=state.selected?`Provincia selezionata: <strong>${state.selected}</strong> <button type="button" class="province-map-clear" id="provinceMapClear">Mostra tutte</button>`:'Clicca una provincia per applicarla come filtro al Radar.';const clear=$('provinceMapClear');if(clear)clear.onclick=()=>{const q=$('q');if(q&&(!q.value||q.value===state.selected)){q.value='';q.dispatchEvent(new Event('input',{bubbles:true}))}state.selected='';renderSelection();schedule()}}
...
function render(){
  ...
  state.chart.setOption({
    ...
    tooltip:{trigger:'item',formatter:p=>{const d=p.data;if(!d)return`<b>${p.name}</b><br>Nessun progetto nel filtro`;const top=d.items.slice(0,3).map(x=>`${x.name} · ${fmt1.format(x.mw)} MW · ${x.stage}`).join('<br>');return`<b>${p.name}</b><br>${metricLabel()}: <b>${metricFmt(d.value)}</b><br>MW eolici: ${fmt1.format(d.mw)}<br>Progetti: ${fmt0.format(d.projects)}<br>MW E4+: ${fmt1.format(d.e4mw)}<br>E7 in costruzione: ${fmt0.format(d.e7)}<br>Priorità A/A+: ${fmt0.format(d.priority)}<br>Progetti senza contractor esecutivo A1/A2: ${fmt0.format(d.openExec)}${d.bess?`<br>BESS separato: ${fmt1.format(d.bess)} MW`:''}${top?`<br><br><b>Priorità nel filtro</b><br>${top}`:''}`}},
  ...
  state.chart.off('click');state.chart.on('click',params=>{if(!params?.name)return;const province=normProvince(params.name),q=$('q');state.selected=province;if(q){q.value=province;q.dispatchEvent(new Event('input',{bubbles:true}))}renderSelection()});
}
...
function bind(){document.querySelectorAll('.province-map-metric').forEach(btn=>btn.addEventListener('click',()=>{state.metric=btn.dataset.metric;document.querySelectorAll('.province-map-metric').forEach(x=>x.classList.toggle('active',x===btn));render()}));['q','region','siteType','stage','type','developer','contractor','mwMin','mwMax','window'].forEach(id=>{const el=$(id);if(el)el.addEventListener(el.tagName==='SELECT'?'change':'input',schedule)});const reset=$('resetFilters');if(reset)reset.addEventListener('click',()=>{state.selected='';setTimeout(()=>{renderSelection();schedule()},0)});window.addEventListener('resize',()=>state.chart?.resize())}
