...
function takeoverContractor(){const panel=$('.contractor-panel');if(!panel||renderLock)return false;const oldLabel=panel.querySelector('.contractor-search'),oldView=$('#contractorView'),oldSum=$('#contractorSummary');if(!oldLabel||!oldView||!oldSum)return false;renderLock=true;
...
function watch(){const k=$('#kpis'),opp=$('#opportunityRows'),body=$('#detailBody');if(k)new MutationObserver(refreshKpis).observe(k,{childList:true,subtree:true});if(opp)new MutationObserver(schedule).observe(opp,{childList:true,subtree:true});if(body)new MutationObserver(injectDrawer).observe(body,{childList:true,subtree:true});['region','stage','type','developer','contractor','window'].forEach(id=>document.getElementById(id)?.addEventListener('change',schedule));['mwMin','mwMax','q'].forEach(id=>document.getElementById(id)?.addEventListener('input',schedule));document.getElementById('resetFilters')?.addEventListener('click',schedule)}
