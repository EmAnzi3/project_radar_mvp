...
let overlay={},publicRegister={},byName=new Map(),body=null,timer=null;
async function load(){
  ...
  [overlay,publicRegister]=await Promise.all([
...
function localPublicSources(intel){
 const a=intel?.public_sources||[];return a.map(x=>({level:x.level,entity:x.entity,use:x.use,url:x.url,kind:'target'}));
}
function officialPublicSources(reg){
 return (reg?.sources||[]).map(x=>({level:x.entity,entity:x.title,use:x.use,url:x.url,kind:x.kind,date:x.date}));
}
function publicSources(intel,reg){
 const a=[...localPublicSources(intel),...officialPublicSources(reg)];if(!a.length)return'';
 ...
 return `<h4>Fonti pubbliche territoriali</h4>${identity}<div class="doc2-public">${a.map(x=>`<div class="doc2-public-row"><span class="doc2-level">${esc(x.kind||x.level||'ENTE')}</span><div><b>${esc(x.entity||x.level)}</b>${x.date?`<span>${esc(x.date)}</span>`:''}<span>${esc(x.use||'')}</span>${x.url?`<a href="${esc(x.url)}" target="_blank" rel="noopener">Apri fonte</a>`:''}</div></div>`).join('')}</div>`;
}
function inject(){
 if(!body)return;
 const old=body.querySelector('.docpass2-section');if(old)old.remove();
 const name=$('#detailTitle')?.textContent.trim(),id=byName.get(name),intel=id?overlay.projects?.[id]:null,reg=id?publicRegister.projects?.[id]:null;
 if(!intel&&!reg)return;
 const docs=(intel?.documents||[]).map(docCard).join('');
 const rels=(intel?.relations||[]).map(relationCard).join('');
 const html=`<section class="detail-section docpass2-section"><h3>Deep document & public-source pass</h3>${docs?`<h4>Documenti acquisiti</h4><div class="doc2-list">${docs}</div>`:''}${rels?`<h4>Soggetti tecnici emersi</h4><div class="doc2-list">${rels}</div>`:''}${schedule(intel)}${publicSources(intel,reg)}</section>`;
...
async function init(){
 try{await load();body=$('#detailBody');if(!body)return;new MutationObserver(scheduleInject).observe(body,{childList:true,subtree:true});$('#drawer')&&new MutationObserver(scheduleInject).observe($('#drawer'),{attributes:true,attributeFilter:['class','aria-hidden']});scheduleInject()}catch(err){console.error('docpass2 intelligence init failed',err)}
}
