...
tranche = json.loads((DATA / "company-network-v06b.json").read_text(encoding="utf-8"))
...
# Merge additive tranches by company id, matching the UI behaviour.
by_id = {}
for payload in (base, tranche):
...
raw_ids = [c["id"] for payload in (base, tranche) for c in payload.get("companies", [])]
assert len(raw_ids) == len(set(raw_ids)), "duplicate company ids across v06 tranches"
assert len(companies) >= 48, f"expanded network too small: {len(companies)}"
...
# All nodes in the expansion tranche must be operationally actionable, not just names.
for c in tranche["companies"]:
...
print(
    ...
    f"{len(tranche['companies'])} operational expansion nodes"
)
